...
**核心弧線：** 浸入默背 → 維護鏈內鬼 → 技術修復撞政治牆 → 傳遞賽跑

**進場前提**：B-1 已透過偽裝讀書與初次向達爾學藝取得 `intel_dal_magic_basics`。

| 變體 | 公主視角概要 | 結局 | 關鍵情報 |
|---|---|---|---|
| C-0 | 以認真學生身分進陣核，兩個月默背；察覺助手異常與陣區暗道結構 | Bad End：收網（越線後被處決） | `intel_magic_array_mnemonic` / `intel_array_handed_clerk` / `intel_array_structural_passage` / `intel_chancellor_treason`（初稿） |
| C-1 | 帶達爾進陣核技術修復＋政治傳遞雙線並行；宰相以誣陷翻轉 | Bad End：誣陷入獄 | `intel_chancellor_treason`（確認）/ `intel_narrative_flip` / `intel_king_defense_line` |
| C-2 | 先分別向賽拉斯、布魯諾、達爾埋種子；幽禁→半年→城破→**自選死亡**（破局的抉擇） | Bad End：自選死亡（種子已埋，但觸發必須在場同時） | `intel_partial_leak` |
...
| 達爾與魔法陣 | 以默背指出腐蝕位置，達爾修復＋取證 | C-0 ＋ C-1 |
...
| LOOP 3（C 線） | 魔法陣 | 陣的本體、維護鏈內鬼、修復與敘事翻轉 | 從孤軍鑽研到必須多線並行 |
...
| `intel_partial_leak` | C-2 | 三條線各持一塊——終章多線並行的基礎 |
| `intel_dal_blinded_by_chancellor` | C-1（選用） | 達爾舊創傷／人證——強化情感與動機 |
