...
### 3. 建議確認：所有調查線 B awakening 開場的表情一致性

調查線 B 基底（`angry.dtl`）和各 v1/v2/v3 變體的開場表情目前不一致（有 `hollow` 有 `angry`），建議統一。
